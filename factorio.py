...
for i in range(2, 888, 2):
    ...
    if lanesofpatch % 2:
        ...
        for j in range(lanesofpatch // 2 + 1):
            numberofminer = math.cos(math.asin(
                j * 2 * tilegauge / radiu)) * radiu / 1.5
            print('o' + str(j), int(
                (numberofminer // tilegroup + 1) * tilegroup), numberofminer)
            epmn += int((numberofminer // tilegroup + 1) * tilegroup) * 4
        print(epmn, int(area / 10.5))
    else:
        for j in range(lanesofpatch // 2):
            numberofminer = math.cos(
                math.asin(
                    (j * 2 * tilegauge + tilegauge) / radiu)) * radiu / 1.5
            print('o' + str(j), int(
                (numberofminer // tilegroup + 1) * tilegroup), numberofminer)
            epmn += int((numberofminer // tilegroup + 1) * tilegroup) * 4
